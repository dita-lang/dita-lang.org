...
Replaces an XML element (and its contents), matched by its `id` attribute
value, with a replacement XML fragment. All other bytes in the file —
including unusual whitespace or quoting in untouched tags — are left exactly
as they were; the file is edited by byte-offset splicing, not by
...
multiple elements share the same `id`, all of them are replaced.
...
```sh
uv run python replace_by_attr.py <file> <id-value> <replacement-xml>
```
...
```sh
uv run python replace_by_attr.py /foo/bar/baz.xml inheritance \
