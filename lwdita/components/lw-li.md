...
-   **XDITA**
    ...
    `<li>` inside [`<ol>`](lw-ol.md) or [`<ul>`](lw-ul.md)
...
-   **MDITA \(core and extended profiles\)**
    ...
    `-`, `+`, or `*` for unordered list, and `0`-`9` and `.` or `)` for ordered list
...
The following example demonstrates the use of list item \(ordered list\) in an MDITA topic.
