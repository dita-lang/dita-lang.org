...
3.  The third item specifies `product="myProduct"`, but already has an `@outputclass` attribute specified in the source. Based on the DITAVAL document, the content is processed as if the original element was: `<li product="myProduct" outputclass="myProductToken example">`. Note that the value supplied by the DITAVAL document is placed before any value already in the source.
