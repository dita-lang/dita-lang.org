...
**Note:** In some cases, an attribute might mean different things on different elements. If an attribute does not use the common definition for a given element, it will be noted in the topic describing that element. exampleFor example, `@type` is often used on linking elements to describe the target of the link, but it is also used on `<note>` elements to describe the type of note.
